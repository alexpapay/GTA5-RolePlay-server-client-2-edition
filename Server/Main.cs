            ...
            if (LoadingFinished)
            {
                if (DateTime.Now.Subtract(_minuteAnnounce).TotalMinutes >= 1)
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (_tick == 0) API.sendChatMessageToAll("~r~Начался капт сектора: " + startCapting.Sector);
                            else API.sendChatMessageToAll("~r~Идет капт сектора: " + startCapting.Sector + " | " + _tick + " минут");
