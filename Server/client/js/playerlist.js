...
API.onServerEventTrigger.connect(function (name, args) {
    if (name == "playerlist") {
        ...
        for (var i = 0; i < list.Count; i++) {
            ...
            newPlayer.userId = obj.userID;
            newPlayer.userName = obj.userName;
        ...
        if (existingPlayer != null) {
            ...
            existingPlayer.userID = args[3];
            existingPlayer.userName = args[4];
        }
        else {
            var newPlayer = new PlayerInfo();
            ...
            newPlayer.userID = args[3];
            newPlayer.userName = args[4];
            g_players.push(newPlayer);
        }
    }
...
API.onUpdate.connect(function () {
    ...
    if (g_currentState == 1) {
        ...
        // Header
        API.drawText("ID  |  User Name  |  SocialName", listX + listPadding, listY - listPadding / 2, 0.4, 255, 255, 0, 255, 0, 0, true, true, 0);
        ...
        for (var i = 0; i < listPageCount; i++) {
            ...
            API.drawText(player.userID + "  |  " + player.userName + " | " + player.name, listX + listPadding, listY + listPadding / 2 + listLine * (i + 1), 0.45, color[0], color[1], color[2], 255, 4, 0, true, true, 0);
